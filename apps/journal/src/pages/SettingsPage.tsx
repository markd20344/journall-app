...
import { ACCENT_PRESETS, getStoredAccentColor, setStoredAccentColor } from "@journall/shared/lib/theme";
import { signOutUser, watchAuthState } from "@journall/shared/firebase/auth";
import { firebaseEnabled } from "@journall/shared/firebase/config";
import { refreshNow } from "../firebase/sync";
import { showToast } from "@journall/shared/lib/toast";
import { cancelPendingDelete, schedulePendingDelete } from "@journall/shared/lib/pendingDelete";
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    void getStoredAccentColor(db.settings).then(setAccentColor);
  ...
  async function chooseAccent(hex: string) {
    setAccentColor(hex);
    await setStoredAccentColor(db.settings, hex);
  }
