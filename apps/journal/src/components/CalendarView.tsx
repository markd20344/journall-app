...
import { useCalendarDatesInRange } from "../hooks/useJournalData";
...
export default function CalendarView({ selectedDate, onSelectDate }: Props) {
  ...
  const scheduledDates = useCalendarDatesInRange(format(gridStart, "yyyy-MM-dd"), format(gridEnd, "yyyy-MM-dd"));
        ...
        {days.map((day) => {
          ...
          return (
            ...
            >
              ...
              {hasScheduled && <span className="calendar-dot" />}
            </button>
          );
        })}
      </div>
