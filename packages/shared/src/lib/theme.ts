...
// Each app has its own Dexie settings table (different schema, different
// database) — this just needs the get/put shape Dexie's Table already has,
// so every app can pass its own `db.settings` straight through.
export interface SettingsTable {
  get: (key: string) => Promise<{ key: string; value: unknown } | undefined>;
  put: (record: { key: string; value: unknown }) => Promise<unknown>;
}
...
export async function getStoredAccentColor(settings: SettingsTable): Promise<string> {
  const record = await settings.get(SETTINGS_KEY);
  return (record?.value as string | undefined) ?? DEFAULT_ACCENT;
}

export async function setStoredAccentColor(settings: SettingsTable, hex: string): Promise<void> {
  await settings.put({ key: SETTINGS_KEY, value: hex });
  applyAccentColor(hex);
}

/** Call once on app startup to apply the saved (or default) accent color. */
export async function applyStoredAccentColor(settings: SettingsTable): Promise<void> {
  const hex = await getStoredAccentColor(settings);
  applyAccentColor(hex);
}
